  ...
  const toggleCategory = (categoryId: string, categories: Category[]): Category[] => {
    return categories.map((category) => {
      if (category.id === categoryId) {
        // Toggle the expansion state of this category
  ...
  const filterCategories = (categories: Category[], term: string): Category[] => {
    ...
    return categories
      .map((category) => {
        ...
        // Return null if this category and its subcategories don't match
        return null
      })
      .filter((category): category is Category => category !== null) // Filter out null values
  }
